...
export default function AdminCart() {
  ...
  const placeOrder = async () => {
    if (isPlacing) return;
    if (!table?.trim() && !isTakeaway) return;
    ...
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center p-6 text-center">
        ...
        <div className="mt-8 flex flex-col gap-3 w-full max-w-xs">
          <button onClick={() => navigate(`/admin/order-summary?table=${placedDetails?.table}`)} className="bg-slate-900 text-white px-8 py-4 rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl active:scale-95 transition-all">View Active Bill</button>
          <button onClick={() => navigate("/admin/tables")} className="bg-slate-50 text-slate-400 px-8 py-4 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-slate-100 transition-all">Floor Plan</button>
        </div>
      </div>
    );
  }
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <textarea 
                ...
                className="w-full p-4 rounded-2xl bg-white border border-slate-100 text-sm outline-none min-h-25"
              />
            </div>
            ...
            >
              ...
              {!isPlacing ? (
                <motion.div
                  ...
                  className="relative z-10 w-16 h-16 rounded-full flex items-center justify-center cursor-grab bg-indigo-500 text-white shadow-lg active:cursor-grabbing"
                >
