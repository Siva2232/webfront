...
export default function OrderBill() {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] pb-20 font-mono relative">
      ...
      <AnimatePresence>
        {markPaidModal && (
          <ModalOverlay onClose={() => !isSubmitting && setMarkPaidModal(null)}>
            ...
            <h3 className="text-lg font-black text-center text-slate-900 mb-2">Record Payment</h3>
            <p className="text-sm text-slate-500 text-center mb-6">
              Total Amount: <span className="font-bold text-slate-900 text-lg">₹{markPaidModal.amount.toLocaleString()}</span>
            </p>
            ...
            <div className="space-y-4 mb-6">
              {[
                ...
                { label: "Balance (Credit)", key: "balance", icon: <AlertCircle size={16} /> },
              ].map((field) => (
                <div key={field.key}>
                  ...
                  <input
                    ...
                    onChange={(e) => setPaymentData({ ...paymentData, [field.key]: Number(e.target.value) })}
              ...
              <div className="p-4 bg-slate-900 rounded-xl">
                <div className="flex justify-between items-center text-white">
                  <span className="text-xs font-bold uppercase opacity-60">Net Entered</span>
                  <span className={`text-lg font-black ${
                    Math.abs((Number(paymentData.cash) + Number(paymentData.bank) + Number(paymentData.discount) - Number(paymentData.balance)) - markPaidModal.amount) < 0.01
                      ? "text-emerald-400"
                      : "text-rose-400"
                  }`}>
                    ₹{(Number(paymentData.cash) + Number(paymentData.bank) + Number(paymentData.discount) - Number(paymentData.balance)).toLocaleString()}
                  </span>
                </div>
              </div>
            </div>
        ...
        {closeBillModal && (
          <ModalOverlay onClose={() => setCloseBillModal(null)}>
            ...
            <h3 className="text-lg font-black text-center text-slate-900 mb-2">
              {closeBillModal.order.table === TAKEAWAY_TABLE || !closeBillModal.order.table ? "Close Bill" : "Close Bill & Free Table"}
            </h3>
          ...
          <>
            ...
            >
              {isClosing ? (
                ...
                <><CheckCircle size={14} /> {order.table === TAKEAWAY_TABLE || !order.table ? "Close Bill" : "Close & Free Table"} <span className="opacity-60 font-medium normal-case text-[9px]">(mark paid first)</span></>
              ) : (
                <><CheckCircle size={14} /> {order.table === TAKEAWAY_TABLE || !order.table ? "Close Bill" : "Close & Free Table"}</>
              )}
            </button>
          </>
        )}
