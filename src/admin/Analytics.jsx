...
export default function Analytics() {
  ...
  const accNetMarginPct = useMemo(() => {
    const inc = Number(accSummary.totalIncome) || 0;
    if (inc <= 0) return null;
    return (Number(accSummary.netProfit) / inc) * 100;
  }, [accSummary.totalIncome, accSummary.netProfit]);
            ...
            {accLoading && !accData ? (
              ...
              <>
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
                  {[
                    { label: "Net profit", val: accSummary.netProfit, icon: Activity, tone: "text-zinc-900" },
                    { label: "Revenue", val: accSummary.totalIncome, icon: TrendingUp, tone: "text-emerald-800" },
                    { label: "Expenses", val: accSummary.totalExpenses, icon: TrendingDown, tone: "text-rose-800" },
                    { label: "Cash", val: accSummary.cashInHand, icon: Wallet, tone: "text-zinc-900" },
                    { label: "Bank", val: accSummary.bankBalance, icon: IndianRupee, tone: "text-zinc-900" },
                    {
                      label: "Net margin",
                      val: accNetMarginPct != null ? `${accNetMarginPct.toFixed(1)}%` : "—",
                      icon: Percent,
                      tone: "text-zinc-900",
                      isText: true,
                    },
                    ...
                    <div
                      key={row.label}
                      className="rounded-2xl border border-zinc-200/80 bg-white p-4 shadow-sm ring-1 ring-zinc-100/80"
                    >
                      <div className="mb-2 flex items-center justify-between">
                        <span className="text-[9px] font-bold uppercase tracking-wider text-zinc-400">{row.label}</span>
                        <row.icon className="h-4 w-4 text-zinc-300" />
                      </div>
                      <p className={`text-lg font-black tabular-nums ${row.tone}`}>
                        {row.isText ? row.val : fmtINR(row.val)}
                      </p>
                    </div>
                  ))}
                </div>
