...
export default function KitchenDashboard() {
  ...
  return (
    <div className="bg-[#FDFDFD] p-4 font-sans text-slate-900 sm:p-6">
      <header className="mb-6 sm:mb-8">
        <h1 className="text-2xl font-black sm:text-4xl">Kitchen Dashboard</h1>
        <p className="text-sm text-slate-500 sm:text-base">Realtime overview for the kitchen team</p>
      </header>

      <div className="mb-8 grid grid-cols-2 gap-2 sm:gap-4 sm:mb-10 lg:grid-cols-6">
      ...
      <section>
        <h2 className="mb-4 text-xl font-black sm:text-2xl">Recent Active Orders</h2>
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:gap-4">
          <button
            type="button"
            onClick={() => navigate("/kitchen/kot")}
            className="w-full rounded-xl bg-indigo-500 px-6 py-3 text-xs font-black uppercase tracking-widest text-white shadow-lg shadow-indigo-100 transition-all hover:bg-indigo-600 sm:w-auto"
          >
            Kitchen Bills
          </button>
          <button
            ...
            className="w-full rounded-xl bg-orange-500 px-6 py-3 text-xs font-black uppercase tracking-widest text-white shadow-lg shadow-orange-100 transition-all hover:bg-orange-600 sm:w-auto"
          >
            Full Orders View
          </button>
        </div>
...
function StatCard({ label, value, icon: Icon, color }) {
  return (
    <div className={`flex flex-col items-center rounded-2xl p-3 shadow-sm sm:p-6 ${color}`}>
      <Icon size={20} className="mb-1 text-slate-700 sm:mb-2 sm:h-6 sm:w-6" />
      <span className="text-xl font-black text-slate-900 sm:text-3xl">{value}</span>
      <span className="mt-0.5 text-center text-[9px] font-bold uppercase text-slate-500 sm:mt-1 sm:text-xs">{label}</span>
    </div>
  );
}
